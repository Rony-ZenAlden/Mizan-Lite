...
| L8 | [L8 §2.3, §15, §17](phases/L8_RELEASE.md) A-L8.1–3, D-L8.i1–i21 | **end-to-end testing in scope**: the built frontend against the real Go graph through a test-only loopback bridge, ten journeys in both languages and a visual pack of every screen, in CI; the counter lands on the Till and Status becomes **About**, with the backups' state warning from every screen; **one date rule everywhere — DD/MM/YYYY, 24-hour, digits only**; every value put into an Arabic sentence isolated by the translator; **an Excel import** for the first products and stock, its preview the import itself rolled back; the support file and the third-party notices; the shop's guides written as Markdown and rendered into the application as PDFs; **the upgrade matrix over every past schema** on every run; the two installers, WebView2 inside the Windows one, signing opt-in; **no JWT and no login** — the owner PIN model of L1 confirmed as the whole of the edition's authentication (D-L8.i21) |
