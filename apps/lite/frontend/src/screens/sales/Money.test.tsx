...
import { DUAL_SEPARATOR } from "@/i18n/numbers";
...
describe("Money — both readings at once (L10, 2026-09-17)", () => {
  it("shows the new figure as the amount and the old one small beside it", async () => {
    renderWithProviders(<Money value={`150${DUAL_SEPARATOR}15000`} currency="SYP" />, { locale: "en" });
    await settle();
    ...
    // Both are grouped as figures, not printed raw with the separator in them.
    expect(shown.textContent).not.toContain(DUAL_SEPARATOR);
  });
