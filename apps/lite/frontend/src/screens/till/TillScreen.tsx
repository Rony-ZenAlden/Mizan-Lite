...
import type { CartInput, CartQuote, Product, Sale, Unit } from "@/api/client";
...
export function TillScreen() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([client.catalog.products({ text: "", includeInactive: false }), client.catalog.units()])
      .then(([p, u]) => {
        if (cancelled) return;
        setProducts(p);
  ...
  const input: CartInput = useMemo(
    () => ({
      lines: cart.map((l) => ({ productId: l.productId, quantity: l.quantity, discountPercent: l.discountPercent })),
      settlement,
      saleDiscount,
      tenderCurrency,
      tendered,
    ...
    [cart, settlement, saleDiscount, tenderCurrency, tendered, changeCurrency],
  );
  ...
  const name = (p: { nameAr: string; nameEn: string }) => (locale === "en" && p.nameEn ? p.nameEn : p.nameAr);
  ...
  const scan = async (event: FormEvent) => {
    ...
    try {
      ...
      const matches = await client.catalog.products({ text: typed, includeInactive: false });
      if (matches.length === 0) {
        setResults(null);
        setNotFound(typed);
      } else {
  ...
  const pay = useCallback(
    async (event?: FormEvent) => {
      ...
      setPayError(null);
      try {
        const sale = await withOwner(() => client.till.checkout({ cart: input, token: quote.token, payment: "cash" }));
        setReceipt(sale);
        setCart([]);
        setSaleDiscount("");
        setTendered("");
  ...
  const quick = useMemo(() => products.filter((p) => p.quickSlot > 0).sort((a, b) => a.quickSlot - b.quickSlot), [products]);
  ...
  const formProblem = quoteError instanceof BindingError && quoteError.code === CODE_NO_RATE ? null : errors.form;
  const other = quote ? quote.otherCurrency : "";
  const canPay = Boolean(quote) && !pending && !busy && !noRate && cart.length > 0;

  return (
    <section className="space-y-4">
      <header className="flex items-center justify-between gap-4">
        <h2 className="text-xl font-semibold">{t("till.title")}</h2>
        {cart.length > 0 ? <Button onClick={clear}>{t("till.clear")}</Button> : null}
      </header>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        <div className="space-y-4">
          ...
          {notFound ? <p role="status" className="text-sm text-danger">{t("till.not_found", { code: notFound })}</p> : null}
          {results ? (
            <ul aria-label={t("till.results")} className="space-y-1">
              {results.map((p) => (
                <li key={p.id}>
                  <Button className="w-full justify-start" onClick={() => add(pickOf(p))}>
                    {name(p)}
                  </Button>
                </li>
              ))}
          ...
          {quick.length > 0 ? (
            <div role="group" aria-label={t("till.quick")} className="grid grid-cols-3 gap-2 xl:grid-cols-4">
              {quick.map((p) => (
                <button
                  key={p.id}
                  type="button"
        ...
        <div className="space-y-4">
          {cart.length === 0 ? (
            <p className="rounded-md border border-dashed border-border p-6 text-center text-text-muted">{t("till.empty")}</p>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {cart.map((l, index) => {
                    ...
                    return (
                      <tr key={l.key} className="border-t border-border align-top" data-testid="cart-line">
                        <td className="p-2">
                          <p className="font-medium">{name(l)}</p>
                          {priced ? (
                            <p className="text-xs text-text-muted">
                              <bdi dir="ltr">
                                {formatDecimal(priced.unitPrice, locale)} {tDynamic(`currency.short.${priced.priceCurrency}`)} / {tDynamic(`uom.${l.unitCode}`)}
                              </bdi>
                            </p>
                          ) : null}
                          {productProblem ? <p role="alert" className="text-xs text-danger">{productProblem}</p> : null}
                          {priced?.warnings.map((w) => (
                            <p key={w} className="text-xs text-danger">
                              {tDynamic(`till.warning.${w}`, { onHand: formatDecimal(priced.onHand, locale), unit: tDynamic(`uom.${l.unitCode}`) })}
                            </p>
                          ))}
                        </td>
                        <td className="w-28 p-2">
                          <CellInput
                            aria-label={t("till.quantity_of", { name: name(l) })}
                            value={l.quantity}
                        ...
                        <td className="w-24 p-2">
                          <CellInput
                            aria-label={t("till.discount_of", { name: name(l) })}
                        ...
                        <td className="p-2">
                          {priced ? (
                            <>
                              <p>
                                <Money value={priced.netLocal} currency={quote!.localCurrency} />
                              </p>
                              <p className="text-xs text-text-muted">
                        ...
                        <td className="p-2">
                          <Button onClick={() => remove(l.key)} aria-label={t("till.remove", { name: name(l) })}>
                            ×
                          </Button>
                        </td>
                      </tr>
          ...
          {payError ? <Alert tone="danger" title={errorText(payError)} /> : null}

          <form className="space-y-3 rounded-lg border border-border bg-surface-raised p-4" onSubmit={(e) => void pay(e)}>
            <div role="group" aria-label={t("till.settlement")} className="flex flex-wrap gap-2">
              {[local, "USD"].filter(Boolean).map((currency) => {
                ...
                return (
                  ...
                  >
                    {t("till.settle_in", { currency: tDynamic(`currency.${currency}`) })}
                  </button>
                );
              })}
            </div>

            <div data-testid="till-total" aria-live="polite" className="space-y-1">
              {quote ? (
                <>
                  ...
                  <p className="text-sm text-text-muted">
                    {t("till.total_other")} <Money value={quote.totalOther} currency={other} />
                  </p>
                  {!isZero(quote.rounding) ? (
                    <p className="text-sm text-text-muted">
                      {t("till.rounding", { note: formatDecimal(quote.cashNote, locale) })} <Money value={quote.rounding} currency={quote.settlement} />
                    </p>
                  ) : null}
                  {quote.discounted ? <p className="text-sm text-danger">{t("till.discount_needs_owner")}</p> : null}
                  <p className={`text-xs ${quote.rateStale ? "text-danger" : "text-text-muted"}`}>
                    <bdi dir="ltr">{t("till.rate", { rate: formatDecimal(quote.rate, locale), currency: tDynamic(`currency.short.${quote.localCurrency}`) })}</bdi>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <TextField
                label={t("till.sale_discount", { currency: tDynamic(`currency.${settlement || local}`) })}
                value={saleDiscount}
                ...
                autoComplete="off"
              />
              <SelectField label={t("till.tender_currency")} value={tenderCurrency} onChange={(e) => setTenderCurrency(e.target.value)}>
              ...
              <TextField
                label={t("till.tendered")}
                hint={t("till.tendered_hint")}
                value={tendered}
            ...
            {quote && quote.tenderGiven ? (
              <p data-testid="till-change" className="text-xl font-semibold">
                {t("till.change")} <Money value={quote.change} currency={quote.changeCurrency} />
              </p>
            ) : null}

            <Button variant="primary" type="submit" className="w-full min-h-12 text-lg" disabled={!canPay}>
              {t("till.pay")}
            </Button>
          </form>
        </div>
