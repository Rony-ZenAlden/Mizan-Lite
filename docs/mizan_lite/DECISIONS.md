...
## 0.9.7 — the counter's day, and what comes back (approved 2026-09-20)

| ID | Decision | Status | Where |
|---|---|---|---|
| D-097.1 | **A return is its own document, not a negative sale and not a void.** `sale_lines` holds `quantity_micro > 0` and every amount `>= 0`; those are real invariants of a thing that happened, and loosening them for a different kind of row would weaken them on every honest row | approved | `sale_returns`, `sale_return_lines` |
| D-097.2 | A return is dated to the day the goods **came back**, not the day of the sale. Monday's report has been read, counted and possibly printed; Thursday's tin is Thursday's business | approved | `reports/domain.ReturnsOn` |
| D-097.3 | The profit a return costs is the **margin, not the refund**: the shop handed back 48,750 and put 30,000 of stock on the shelf, so it is out 18,750 | approved | `Returns.Profit()` |
| D-097.4 | A returned line is worth its share of the line's **net** — after that line's discount. The whole line back takes no division at all, so the parts of a fully returned sale add to exactly what was charged | approved | `PriceReturn` |
| D-097.5 | Cost re-credits at the cost the **sale snapshotted**, not today's average (D-L9.1's reasoning). A line whose cost was never known re-credits nothing and says so | approved | `PriceReturn`, `Returns.Unknown` |
| D-097.6 | A debt-settled return needs its **own ledger kind**. `write_off` was the obvious fit and would have been wrong: the reports count write-offs as BAD DEBTS, so a returning customer would be recorded as a defaulter and the event counted twice | approved — found by reading `BadDebtsOn` | `customers/domain.KindSaleReturn` |
| D-097.7 | A cash return writes **no cash entry**. A cash *sale* writes none either: the drawer derives both. One would have made the drawer short by every refund twice over | approved | `DrawerTerms.ReturnsOut` |
| D-097.8 | A line may be returned **more than once**, so `ux_stock_ledger_sale_line_kind` becomes partial — still one sale and one void per line, any number of returns | approved | migration 0010 |
| D-097.9 | With **no printer configured the till shows no receipt panel**: it stood between the cashier and the next customer with nothing to offer. The sale is recorded either way and the receipt is one click away | approved — the owner's report | `TillScreen` |
| D-097.10 | The **open cart is kept in the webview's storage**, not the database. Nothing has been sold; a row in the books that no report may count would make every reader learn to skip it. Prices are NOT kept — Go prices it again, because the rate moves daily | approved | `till/openCart.ts` |
| D-097.11 | A recurring expense is a **marking, not a schedule**. Nothing is posted automatically: the drawer has to match the cash in it, and an app that posted the rent on the first would show money leaving a drawer nobody opened | approved, over an auto-posting alternative | `cashbook/domain.Recurrence` |
| D-097.12 | **Low stock is a level the shop types**, per product, in the product's own unit. "Low" is a comparison and only the shop knows the second number; a product with no level is never called low | approved | `catalog/domain.SetReorder` |
...
## 0.9.9 — held sales, the open item, re-pricing and one notification engine (approved 2026-09-23)

The owner approved three features (hold and resume, an open-priced item, an optional re-price when the rate moves) and one
architectural requirement (a unified notification system), and rejected six others; see the project memory. Building it
found three input bugs in the redenomination pipeline, the first of them shipped since 0.9.5.

| ID | Decision | Status | Where |
|---|---|---|---|
| D-099.1 | An **open-priced item** is a product flag, fixed at creation, sold at a price the cashier types each time and **never counted in stock**. The database refuses a stock movement for one (trigger), the stock service refuses it with its own code, and it can have no reorder level | approved | migration 0011 §1, `stock.ErrOpenPriceNoStock` |
| D-099.2 | The till's **Misc** item (متفرقات) is **created the first time Misc is pressed**, in the shop's own currency, not seeded by the migration: a seeded row turned up in every fresh database and every contract test. A shop that deactivated it, or gave its name to another product, is refused rather than overruled | approved | `catalog.OpenItem` |
| D-099.3 | An open item's sales are **outside revenue and profit** like a line with no cost, but reported **apart and plainly** — "open price", not a missing cost. A void moves no stock for them, a return never restocks them, and the stock check expects no movement | approved | `reports/domain.Open`, `sales.verify` |
| D-099.4 | An open item's price is typed **in the product's own currency**, and the till says which: a dollar-priced open item asked for "Price (SYP)" while Go read the figure as dollars | approved — found while testing | `ScanDTO.PriceCurrency`, `OpenItemDialog` |
| D-099.5 | **Held sales** live in the webview's storage, like the open cart (D-097.10): at most nine, what was picked and never a price. Resuming one holds the open cart first, so nothing the cashier was doing is thrown away. F6 holds | approved | `till/openCart.ts` |
| D-099.6 | Each product remembers the **rate it was priced at** (`priced_rate_nano`), set whenever its price or currency changes. Existing products are backfilled with the rate of the business day of their last change, or the shop's first rate | approved | migration 0011 §2 |
| D-099.7 | A local-currency price is **stale** when the rate has moved **5% or more** since it was priced, either way. A dollar price never is | approved | `catalog/domain.Stale` |
| D-099.8 | **Re-pricing is a proposal.** Each proposed price keeps the product's dollar value (old × now ÷ then), rounded to the cash note and never below one note; or the owner moves every price by one percentage (±90). Nothing changes until the owner confirms, each change is its own owner event at the version the owner saw, and closing the dialog changes nothing | approved — the owner: "never mandatory or auto-applied" | `catalog.RepriceProposal`, `catalog.BulkReprice` |
| D-099.9 | **One notification engine**, in Go: low stock, stale prices (with those now below their replacement cost), the capital, and the shop's own backups. One call answers the bell, the toasts and the notification centre, so the three cannot disagree | approved | `internal/lite/alerts` |
| D-099.10 | **Capital** is the stock at its dollar cost, the drawer and what customers owe, in dollars at each day's rate, recorded once a day (rewritten at most every 30 minutes) **whether or not anyone is in owner mode** — recording is not disclosing. It is compared with about a month back, once there is a week of history. The owner is told at a **5%** move in the dollar total, or when the pounds the shop held lost **2%** of its worth to the rate: a loss with nothing happening in the shop has no noise to allow for | approved | `alerts/domain.CompareCapital` |
| D-099.11 | The owner's figures — capital, replacement costs, which prices sell at a loss — are **computed always and disclosed only in owner mode** when the PIN is on. The list is rebuilt from what may be shown, so the bell never counts what the centre may not show | approved | `alerts.withholdOwnerFigures` |
| D-099.12 | **Read** is a notification's key and fingerprint, kept in the webview. Go changes the fingerprint when the condition changes materially: a new rate, a capital move into the next 5% band, a new day without a backup. Whether a price sells at a loss is not in it, or every unlock and lock would announce the same list | approved | `Facts.Notifications`, `NotificationProvider` |
| D-099.13 | **Toasts** announce only what is new and unread, for eight seconds, three at most; the first look of a session is one summary. They are polite and in a corner, never modal. The engine is asked every minute, on every change of screen, on a new rate and after each sale | approved | `NotificationProvider`, `Toasts` |
| D-099.14 | **The backup banner leaves the counter.** A missing or old backup is a notification like the rest, and comes back **once a day** (the business date is its fingerprint) until it is dealt with — a daily reminder instead of a red bar across the till | approved — follows D-098.1 | `Shell.tsx`, `alerts/domain` |
| D-099.15 | **Bug, shipped in 0.9.5:** with the new or dual display on, amounts typed at the till (tendered, sale discount), for a customer (payment, opening balance, write-off, refund) and on a delivery (cost, rate) reached the books **without being taken back to old pounds** — 150 typed was stored as 150 old pounds, not 15,000. Every typed amount now goes through `Base`, and a gate test types each one | fixed | `l11_input_test.go` |
| D-099.16 | **Bug:** `Base` moved the point in the text as typed, which knows only `.`, so `١٫٥` new pounds became 1.5 old pounds instead of 150. It now reads the figure with `numinput.Normalise` first; `moneyfmt` may import `numinput`, which is as pure as it is | fixed | `moneyfmt.Base`, `arch-rules.yml` |
| D-099.17 | **Bug:** a form prefilled in dual mode with "150 (15000)" could not be saved — that is a reading, not a number. A prefill takes the new figure, which is what a person types | fixed | `numbers.typeable` |
| D-099.18 | **Bug, found by J12:** F9 pressed in the instant after a line was added did nothing, silently. The till marked the cart "being priced" in an effect, which runs after the render, so a key in between saw a cart neither priced nor being priced; and the effect that cancelled a pending F9 on a cart change could run late and cancel an F9 made after that change. The till now knows **which cart its quote answers**, derived in the render itself, and a pending F9 names the cart it was pressed for | fixed | `TillScreen` (`beingPriced`, `payFor`) |

## 0.10.0 — the invoice, suppliers, spoilage and US dollars only (approved 2026-09-24)

The owner's request of 2026-09-24, with a reference invoice ("المروى") as the blueprint, a correction making all branding
dynamic, and four answers: US dollars only converts everything; money paid to a supplier comes from the drawer or the
owner's own pocket, chosen each time; a supplier does not charge for goods that arrive damaged; the invoice is an A4
page beside the 80 mm receipt. Built in four parts on one migration (0012), which is why no intermediate build was
installed on the owner's data: a part-way migration would have been recorded as applied.

| ID | Decision | Status | Where |
|---|---|---|---|
| D-0100.1 | **The A4 invoice** follows the reference: store name and logo, invoice number, customer (السيد), date, city (الجهة) and phone above; the columns الإجمالي · الصنف · الكمية · الوحدة · طرد · ملاحظات · السعر; the carton count, the total, the amount in words (التفقيط) and the net below. It prints on the A4 printer chosen on the Printer screen, or saves as a PDF | approved | `api/printouts.go`, `documents` |
| D-0100.2 | **Branding is the shop's own, never the program's:** name, phone, city and address in Settings, and a PNG or JPG logo the owner uploads, shrunk to 800 px and kept **inside the database** so a backup carries it. With no logo the name is set in type | approved — the owner's correction | migration 0012 §1, `settings/domain.NormaliseLogo` |
| D-0100.3 | **طرد** is a product's units per carton, copied onto each sale line; the column is blank for a product without one. **ملاحظات** shows a line's discount | approved | migration 0012 §2 |
| D-0100.4 | **The amount in words** is Go's (`tafqeet`), in the settlement currency and its real decimals — cents for dollars — as the reading the shop uses first | approved | `internal/lite/tafqeet` |
| D-0100.5 | **Suppliers are a book of their own**, apart from the customers': a customer's balance is owed to the shop and a supplier's by it, and one book would net the two. Above nought the shop owes; below, the supplier owes the shop | approved | `internal/lite/suppliers` |
| D-0100.6 | **A purchase** has lines — how many arrived, how many damaged, the price of one, a discount as a percentage or an amount — and a discount on the whole invoice shared over the lines by what each costs (largest remainder). Go quotes it as it is typed | approved | `suppliers/domain.Price` |
| D-0100.7 | **Damaged units are never received and never charged**: the stock book takes the good units only, and money already paid for damaged ones is the supplier owing the shop, which a refund settles | approved — the owner's answer | `suppliers.RecordPurchase` |
| D-0100.8 | **Good units are received through the stock book at what they cost after every discount**, so the average cost — and every margin — reads the real cost. A void reverses the receipts while each is its product's newest movement, and otherwise says which line stands in the way | approved | `suppliers.Stock` port |
| D-0100.9 | **Money paid to a supplier** comes from the drawer or the owner's pocket, **chosen each time**; only the drawer's moves the drawer's expected cash (a line of its own, as does a supplier's refund) | approved — the owner's answer | `reports/domain.DrawerTerms` |
| D-0100.10 | **What the shop owes its suppliers comes off its capital**: a delivery on credit adds stock and a debt of the same size. Pounds owed are pounds a falling pound makes cheaper | approved | `alerts/domain.Snapshot` |
| D-0100.11 | With the PIN switch on, **the suppliers' book is the owner's**, screens and all: every figure in it is a cost or a debt | approved | `suppliers.visible` |
| D-0100.12 | **A supplier's discount** is also offered on a plain stock delivery and on the product form's cost price, each worked out by Go; the product form's discount comes off the cost typed with it, never off a stored one | approved | `stock/domain.discounted`, `catalog/domain.DiscountCost` |
| D-0100.13 | **Spoiled** — food gone bad before its date — is a write-off reason beside damaged and expired. The stock ledger is rebuilt for it (0012 §4), before the supplier tables that point at it | approved | migration 0012 §4 |
| D-0100.14 | **The spoilage screen** records a write-off through the stock book's own owner-guarded act, and reads the period's losses at cost — dollars at each movement's cost, pounds at its day's rate — by reason and by line. Goods that arrived damaged are listed apart at what they would have cost: not charged, so **shown and not counted** | approved | `reports/domain.LossesBetween` |
| D-0100.15 | **US dollars only is a fourth reading of the money**, reached only through the switch, which **converts everything** at the rate in force: prices and costs (a price under half a cent becomes a cent, shown first), the Misc item's currency, every customer's and supplier's pound balance, and the drawer's pounds — in one transaction and one owner's act | approved — the owner's answer | `internal/lite/usdmode` |
| D-0100.16 | **The owner confirms the plan they were shown**: a token names every figure, and a sale or payment in between is refused with the new figures, never converted quietly | approved | `usdmode/domain.Plan.Token` |
| D-0100.17 | A balance moves between currencies by a **conversion** entry pair — not an opening (always above nought) and not a write-off (a bad debt). The debt ledger is rebuilt for the kind (0012 §7), with voucher numbers and sale returns copied aside and back; a planted debt-settled return proves it. A conversion is never reversed | approved | migration 0012 §7 |
| D-0100.18 | While dollars-only, **Go refuses pounds at every door** — the till (which defaults to dollars), prices, deliveries, customer and supplier money, the drawer — and printouts carry no rate. Leaving is a plain setting: everything is in dollars already | approved | `api/usdonly.go` |
| D-0100.19 | **Known limit:** a return of a sale made before the switch is refunded in the currency it was paid in | recorded | `sales.Return` |
| D-0100.20 | **Found on the way, fixed:** a cash return was in the drawer's expected figure on no line of its own; a dual-reading zero did not read as zero, and a negative one kept a minus inside its brackets; an open dialog pulled focus back every second while the owner's countdown ran; a barcode scanned into a form before its products loaded was dropped (found by J13) | fixed | `CashScreen`, `Money.isZero`, `Dialog`, `useProductSearch` |

## 0.10.1 — dollars only everywhere, figures that stay in their cells, the logo on screen, the furniture demo (2026-09-25)

The owner's report of 2026-09-24 after testing 0.10.0: a dollars-only till still showed pounds beside each line; an A4
invoice's total ran out of its column; and a request for a logo for "الكردي" (furniture and home goods), seeded with rich
demo data for a client presentation, and a new build. The owner's instruction was to execute; the choices below were made
from the request and recorded here, the first of them the one that touches the owner's data (D-0101.7).

| ID | Decision | Status | Where |
|---|---|---|---|
| D-0101.1 | **A dollars-only shop reads no pound reading of a dollar figure, anywhere.** Go sends none: a product's converted price (the Products screen, a shelf label), a balance's reference in the other currency, the stock's value in pounds, the losses' pound value, a day's pound takings with nothing in them. The screens drop their pound-only pieces: a till line's pound total (the owner's report), the rate at the foot of a receipt and in the payment dialog, the payment's currency choices, the report's rate note, the Products screen's other-currency column. The end-of-day report prints in dollars, and the day, month, products and stock exports have no pound columns | approved — the owner's report | `api/catalog.go`, `api/customers.go`, `api/stock.go`, `api/losses.go`, `api/reports.go`, `api/printouts.go`, `api/export_reports.go`, `TillScreen`, `ReceiptView`, `PaymentDialog`, `DayStatement`, `ProductsTable`, `ProductsScreen` |
| D-0101.2 | **Records made in pounds stay in pounds.** A sale charged in pounds before the switch, the cash-book entries of the switch itself and a day's takings in pounds are what happened; showing them in dollars would state a rate nobody used. They appear only on the day of the switch and when an earlier day is looked up | approved | — |
| D-0101.3 | **A table's figure stays inside its cell, whatever it holds.** A figure too wide for its column breaks at the bracket of a dual reading — the new pounds over the old — and a line still too wide is set smaller until it fits; nothing is ever cut, because a cut figure reads as another amount. The invoice's total and price columns are wider (16 and 17 of 94) so the figures a shop prints fit at full size | approved — the owner's report | `documents.fitted`, `typeset.Line.Scaled`, `api.invoiceColumns` |
| D-0101.4 | **The A4 preview is the printer's own page**, drawn at its 200 dpi and scaled down by the screen, pixel for pixel what a Windows driver receives. At 110 dpi the address under the shop's name lost the thin strokes of alef and lam to the black-and-white threshold | approved | `api.previewDPI` |
| D-0101.5 | **The header shows the shop's own logo** beside its name — whatever the owner uploaded, on a light ground so a dark mark reads in the dark theme — and changes the moment Settings does. The application still holds no shop's logo (D-0100.2) | approved — the owner's request | `shop/ShopProvider`, `Shell` |
| D-0101.6 | **The furniture and home-goods demo is a seeder profile, not part of the application**: `lite-demoseed -profile home`, its catalogue, suppliers and customers in `demoseed/data/home.json`, its logo passed in as an owner's upload. Dollars only from its first day, 45 days of trade through the services, and the same verifiers and reconciliations, which must find nothing. No outside backup folder and no automatic printing, so a presentation never reaches for a folder or a printer this computer does not have. `-export` writes a verified backup for another computer to restore. The pantry demo stays the default: the journeys run on it | approved | `internal/lite/demoseed/home.go`, [DEMO.md](DEMO.md) |
| D-0101.7 | **On the owner's Mac the demo replaced the seeded pantry test shop, which was moved aside whole and untouched** — the request was for the demo to appear at once, and nothing of that shop was changed or removed. [DEMO.md](DEMO.md) says how to go back | approved — the owner's request | [DEMO.md](DEMO.md) |
| D-0101.8 | **The الكردي brand kit** lives in `assets/brands/al-kurdi/`: SVG masters with the lettering outlined from IBM Plex Sans Arabic (OFL — the face the application's documents use), so they open the same in any editor, and PNGs rendered from them. The logo in the shop is the Arabic lockup without the Latin line, which would print as specks on a receipt; the bilingual, stacked, emblem and one-colour versions are for signs and print | approved | `assets/brands/al-kurdi/` |
| D-0101.9 | **Found on the way, fixed:** a dual-reading zero printed a discount of "(0) 0" on receipts and invoices; the end-of-day report printed "{count}" in its first line and labelled its printing time with a screen message; a demo seeded before opening would have dated today's sales after the moment it ran | fixed | `api.isZeroFigure`, `zReportDocument`, `demoseed.today` |
